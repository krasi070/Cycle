namespace Cycle.Models.Attacks.MagicAttacks
{
    ...
    public class DefenseBuffSpell : BuffMagic
    {
        public DefenseBuffSpell(string name, int mpCost, int buffAmount, string textAmount) 
        ...
        public override string[] CastMagic(IUnit user, IUnit target)
        {
            ...
            if (hitTarget)
            {
                int buffedAmount = user.BattleDefense;
                user.BattleDefense += this.BuffAmount;
                buffedAmount = user.BattleDefense - buffedAmount;
                if (buffedAmount > 0)
                {
                    message[1] = string.Format("{0}'s defense {1}.", this.Name, this.TextAmount);
                }
                else
                {
                    message[1] = string.Format("{0} can't increase his defense anymore.", this.Name);
                }
            }
            else
            {
